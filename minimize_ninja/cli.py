...
from minimize_ninja import read_config, configure_logger, get_logger
from minimize_ninja import KeynoteFile, TiffyYaml, ImageFile, Stopwatch


@click.group(help=f'MinimizeNinja version 0.0.2: Personal trainer for '
                  f'Keynote files.')
@click.option('-v', '--verbose', 'verbosity',
              help='Increase verbosity', count=True)
@click.option('--log-file', 'log_file',
              help='Log file to sent all output to')
...
@click.command(help='Get a Keynote file into shape by losing '
                    'unnecessary weight')
@click.option('-q', '--quality', 'quality', default=0, type=int,
              show_default=True,
...
def slim(quality, keynote_file, export_pdf, keep_unpacked,
    ...
    if resize_factor < 2.0:
        console.print()
        logger.warning('You selected a resizing factor of below 2.0! This '
    ...
    if jpeg_compression < 80:
        console.print()
        logger.warning('You selected a JPEG quality setting below 80! This '
    ...
    tiffies = [
        image for id, image in images_dict.items()
        if 'tif' in image.filename.suffix
    ]
    pngs = [
        image for id, image in images_dict.items()
        if 'png' in image.filename.suffix
    ]

    if tiffies:
        logger.info(f'Found {len(tiffies)} TIFF files 🙄. Did someone have'
        ...
        logger.info(f'Building personal training plans to get images into '
                    f'shape…')
        ...
        logger.info(f'…done! Reducing TIFFs with a size of '
    ...
    if pngs and png_convert:
        console.print()
        logger.info(f'Found {len(pngs)} PNG files 🙄. Let us check for '
        ...
        logger.info(f'…done! Reducing PNGs with a size of '
                    f'{sizes_original_humanized} to '
                    f'{sizes_converted_humanized} ({reduction:.1f} %'
                    f' reduction). 🚀')
        metadata.save()
    ...
    console.print()
    logger.info(f"Found {len(images_dict.keys())} tasty images. 🧁 Let's see "
    ...
    logger.debug(f'Searching {len(all_yaml)} metadata YAML files for '
            ...
            for archive in chunk['archives']:
                for object in archive['objects']:
                    ...
                    if object['_pbtype'] == 'KN.SlideStyleArchive':
                        if ('image'
                                in object.get('slideProperties', {})
                                .get('fill', {})):
                            identifier = \
                                object.get('slideProperties', {}) \
                                .get('fill', {}) \
                                .get('image', {}).get('imagedata', {}) \
                                .get('identifier', 0)
                            if identifier in images_dict:
                                images_dict[identifier]\
                                    .add_slide_style_reference(
                                    yaml_file, object)
    ...
    sizes_converted = sum(
        [image.size_converted for id, image in images_dict.items()])
    sizes_resized = sum(
        [image.size_resized for id, image in images_dict.items()])
    sizes_converted_humanized = humanize.naturalsize(sizes_converted)
    ...
    console.print()
    logger.info(f"Analyze nutrients in {len(images_dict.keys())} images. "
    ...
    sizes_resized = sum(
        [image.size_resized for id, image in images_dict.items()])
    sizes_optimized = sum(
        [image.size_optimized for id, image in images_dict.items()])
    sizes_resized_humanized = humanize.naturalsize(sizes_resized)
...
MinimizeNinja version: 0.0.2
        ...
        console.print('\n\n')
        console.print(Panel.fit(crash_message, title=":warning-emoji: [bold red] Oops…[/]"))
